...
ARES používá oficiální REST/JSON službu Ministerstva financí, nikoli SOAP. Produkční klient je oddělen přes `IAresClient`; automatické testy používají lokální HTTP handler a JSON fixtures, takže produkční ARES nekontaktují. Podrobnosti jsou v `docs/architecture/ARES_INTEGRATION.md`. Současný přímý fakturační PDF export je jednostránkový technický výstup, nikoli upravitelná profesionální šablona. Audit datového toku a návrh bezpečných šablon je v `docs/architecture/PRINT_AND_TEMPLATE_ARCHITECTURE.md`.
