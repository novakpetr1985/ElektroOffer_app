...
## 🆕 Co je nového
- **Refaktor persistence**  
  Persistence (file I/O a JSON serializace) byla přesunuta do samostatné vrstvy **`IProjectStorage`** a konkrétní implementace **`FileProjectStorage`**. Tím se jasně odděluje ukládání souborů od orchestrace aplikace.
- **ProjectService jako orchestrátor**  
  `ProjectService` nyní přijímá `IProjectStorage` v konstruktoru a slouží pouze k orchestrace operací (deleguje ukládání/načítání do `_storage`). Zlepšuje to čitelnost kódu a připravuje projekt na použití DI.
- **Lepší testovatelnost**  
  Storage lze snadno mockovat; file I/O testy se přesunuly na `FileProjectStorage`, testy `ProjectService` používají mock `IProjectStorage`.
- **Bezpečnost závislostí**  
  Některé `SQLitePCLRaw` balíčky jsou explicitně pinovány kvůli bezpečnostní opravě.
- **Poznámka pro vývojáře:** Po refaktoru už neexistuje bezparametrový konstruktor `ProjectService()`. Pokud aktualizujete kód, nahraďte volání `new ProjectService()` voláním `new ProjectService(new FileProjectStorage())` nebo injektujte `IProjectStorage` přes DI.
...
| Service                  | Účel                                                 |
| ------------------------ | ---------------------------------------------------- |
| `ProjectService`         | Orchestrace projektových operací (Save/Load/Confirm) |
| `FileProjectStorage`     | Ukládání / načítání souborů + JSON serializace       |
