...
Spouští se při `push`, `pull_request` a manuálně (`workflow_dispatch`). Kroky: checkout → setup .NET → restore → build → test → publish artifact.
...
- [x] GitHub tests - UNIT + integration + build při každé akci
