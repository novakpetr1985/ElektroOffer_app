...
Automatické testy nejsou end-to-end test celé desktopové aplikace. Po sloučení výsledků unit a integrační sady je aktuální řádkové pokrytí produkčního C# kódu přibližně **56 %**. Dobře jsou pokryté kalkulace, repository, SQLite schéma/CRUD, katalog, import XLSX a serializace projektů/faktur. Slabší pokrytí mají `MainViewModel`, `InvoiceViewModel`, ARES, motiv, tisk a code-behind oken. Dialogové smoke testy jsou navíc `[Explicit]` a v běžném CI se nespouštějí.
