...
Lokální SQLite databáze není verzovaná v Gitu (`*.db` je v `.gitignore`). Pokud se smaže `bin`, `obj` nebo původní lokální `.db`, aplikace při startu založí novou databázi podle SQL seedu `ElektroOffer_app/Data/Seed/elektrooffer_1_9_0.sql`. Ten obsahuje aktuální testovací data pro sekce PRÁCE a MATERIÁL.
