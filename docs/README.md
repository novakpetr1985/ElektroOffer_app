...
Lokální testovací databáze `ElektroOffer_app/elektrooffer.db` není verzovaná v Gitu (`*.db` je v `.gitignore`). Pokud ji máš lokálně ve složce projektu, Visual Studio ji při buildu zkopíruje do `bin` podle nastavení v `.csproj`; po smazání `bin/obj` tedy zůstane zdrojem dat právě lokální soubor v projektu.
